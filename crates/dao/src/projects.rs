...
use crate::models::{deployment, project_traffic};
use crate::{
    db::DB,
...
pub async fn get_traffic(project_id: i32) -> Result<Option<TrafficSummary>> {
    let (_, time_at) = get_traffic_hour();
    let db = DB.get().unwrap();
...
pub fn get_traffic_hour() -> (i64, String) {
